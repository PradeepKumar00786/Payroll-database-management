...
WHERE name LIKE 'a%' OR 'e%' OR 'i%' OR 'o%' OR 'u%';
...
-- query 11 = AVG salary OF DATA SCIENTIST
SELECT AVG(salary)
FROM ((emp_info
INNER JOIN designation_info ON emp_info.id = designation_info.id)
...
-- query 13 = select employee name and gender from Lucknow and Noida?
SELECT name, gender, city FROM emp_info
WHERE city IN ('Lucknow', 'Noida');

-- query 14 = degree wise avg salary which degree has avg salary more then 55000
SELECT designation_info.degree, AVG(emp_info.salary)
FROM (designation_info
INNER JOIN emp_info ON designation_info.id = emp_info.id)
GROUP BY degree
HAVING AVG(emp_info.salary) > 55000;

-- query 15 = select name and salary with explaination
SELECT name, salary,
CASE
    WHEN salary > 50000 THEN 'The salary is greater than 50000'
    WHEN salary < 50000 THEN 'The salary is less than 50000'
    ELSE 'The salary is equal to 50000'
END AS salaryText
FROM emp_info;
